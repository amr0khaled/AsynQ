...
export default function AiResponseTextarea({ isStreaming, content, setContent }: Props) {
  ...
  if (isEditing) return <Card className='w-full md:min-w-96 border-transparent pt-0 gap-0'>
          ...
          <Controller
            ...
            render={({ field, fieldState }) => <Field>
              <Textarea
                ...
                className='resize-none min-h-80 max-h-96 md:min-h-64 text-sm md:text-base'
              />
            </Field>
            }
          />
    ...
    <CardFooter className='pt-2 md:pt-6'>
      <FieldGroup className='flex flex-row items-center justify-center md:justify-end'>
        <Button
          className='w-24'
          variant={'outline'}
          type='button'
  ...
  return <div className='flex flex-col items-center w-full mx-auto'>
    <div className='flex flex-col items-center gap-y-4 max-w-full sm:max-w-xl md:max-w-2xl lg:max-w-11/12'>
      <div className="p-3 text-white/85 bg-gray-50 dark:bg-transparent border rounded-lg">
        <div className="text-wrap prose dark:prose-invert max-w-none min-w-0 w-full text-sm md:text-base">
          <ReactMarkDown
            ...
            components={{
              ...
              pre({ className, ...props }) {
                return <pre
                  {...props}
                  className={`${!!className ? className + ' ' : ''}p-0 md:px-[1.14286em]`}
                >
                </pre>
              }
            }}>
